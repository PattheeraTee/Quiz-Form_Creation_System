...
const ResponsePage = ({ quizId, formType }) => {
  ...
  useEffect(() => {
    if (quizId) {
      fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/response/form/${quizId}?type=${formType}`)
  ...
  const fetchDetailResponses = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/response/form/${quizId}/detail?type=${formType}`);
  ...
  const downloadExcelFile = async () => {
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/response/download/${quizId}`, {
        responseType: 'blob',
      });
  ...
  const handleEvaluateWithAI = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/response/text-input/${selectedQuestionId}`);
      const questionData = await response.json();
  
      const previewRes = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/evaluate-preview`, {
        method: "POST",
  ...
  const handleAcceptEvaluation = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/accept-evaluation`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
